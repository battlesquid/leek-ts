...
@ApplyOptions<Subcommand.Options>({
	...
	subcommands: [
		chatInputCommand(name_change_request.commands.chat.subcommands.enable.name),
		chatInputCommand(
			name_change_request.commands.chat.subcommands.disable.name,
		),
	],
	...
	public async chatInputDisable(
		...
		try {
			await this.db
				...
				.where(eq(nameChangeRequestSettings.gid, inter.guildId));
